...
describe("Requests", function () {
    describe("Listing Requests Made by Current User", function () {
        ...
        var listUser = function (user, parameters) {
            return listRequested(parameters, user.accessToken);
        };

        // check it requires authentication
        auth.itRequiresAuthentication(curry(listRequested)({}));
        ...
        describe("with test requests set up", function () {
            ...
            it("returns a successful response", function () {
                return expect(listUser(me)).to.be.a.requested.listSuccess;
            });
            ...
            describe("with limit parameter", function () {
                ...
                it("lets us limit the results to less than 25", function () {
                    return listUser(me, { limit: 15 }).then(function (response) {
                        expect(response).to.be.a.requested.listSuccess;
                        expect(response.body.requested.length).to.equal(15);
                        expect(response.body.count).to.equal(40);
                    });
                });
                ...
                it("lets us limit the results to between 25 and 40", function () {
                    return listUser(me, { limit: 30 }).then(function (response) {
                        expect(response).to.be.a.requested.listSuccess;
                        expect(response.body.requested.length).to.equal(30);
                        expect(response.body.count).to.equal(40);
                    });
                });
                ...
                it("lets us specify a valid offset", function () {
                    return listUser(me).then(function (defResponse) {
                        return listUser(me, { offset: 5 }).then(function (response) {
                            ...
                            expect(response.body.count).to.equal(40);
                            expect(response.body.requested.length).to.equal(25);
                            var offsetResults = response.body.requested.slice(0, 20);
                            var defaultResults = defResponse.body.requested.slice(5);
                ...
                it("lets us specify a valid offset that means less results are returned", function () {
                    return listUser(me).then(function (defResponse) {
                        return listUser(me, { offset: 20 }).then(function (response) {
                            expect(response).to.be.a.requested.listSuccess;
                            expect(response.body.count).to.equal(40);
                            expect(response.body.requested.length).to.equal(20);
                            var offsetResults = response.body.requested.slice(0, 5);
                            var defaultResults = defResponse.body.requested.slice(20);
                            expect(offsetResults).to.deep.equal(defaultResults);
                        });
                    });
                });
                ...
                it("lets us specify both an offset and limit parameter", function () {
                    return listUser(me).then(function (defResponse) {
                        return listUser(me, { offset: 5, limit: 5 }).then(function (response) {
                            expect(response).to.be.a.requested.listSuccess;
                            expect(response.body.count).to.equal(40);
                            expect(response.body.requested.length).to.equal(5);
                            var offsetResults = response.body.requested.slice(0, 5);
                            var defaultResults = defResponse.body.requested.slice(5, 10);
                            expect(offsetResults).to.deep.equal(defaultResults);
                        });
                    });
                });
            ...
            describe("with sort_by parameter", function () {
                it("defaults to sorting by id", function () {
                    return listUser(me).then(function (response) {
                        ...
                        var sorted = response.body.requested.slice(0).sort(function (requestA, requestB) {
                            // numeric IDs
                            return requestA.id - requestB.id;
                        });
                        expect(response.body.requested).to.deep.equal(sorted);
                    });
                });
            ...
            describe("with email parameter", function () {
                ...
                it("handles searching exactly", function () {
                    return listUser(me, { email: "test.email@for.matching.com" }).then(function (response) {
                        expect(response).to.be.a.requested.listSuccess;
                        expect(response.body.count).to.equal(1);
                        expect(response.body.requested.length).to.equal(1);
                        expect(response.body.requested[0].email).to.equal("test.email@for.matching.com");
                    });
                });
