...
function parseError(err, errors) {
    ...
    if (err.name === "ValidatorError" || err.name === "CastError") {
        ...
        } else if (err.kind === "regexp" || (typeof ps !== "undefined" && (ps.kind === "enum" || ps.type === "enum"))) {
...
function errorHandler(err, req, res, next) {
    ...
    if(err.name === "ValidationError" || err.name === "ValidatorError" || err.name === "CastError"){
      try {
        var mongoGeneratedError = parseError(err, []);
        if(mongoGeneratedError.length){
          err = mongoGeneratedError[0];
        }else{
          err = ERRORS.INVALID_SUBMISSION;
        }
      }
      catch(errorz) {
        err = ERRORS.INVALID_SUBMISSION;
    ...
    var error = err; //new APICombinedError(parseError(err, []));
    ...
    res.status(error.responseCode || 500);
    res.send(error);
    // res.send({
    //     success: false,
    //     errors: error.slugs
