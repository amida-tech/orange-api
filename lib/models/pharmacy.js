...
PharmacySchema.methods.setData = function (data) {
    ...
    // if values are passed, use those instead
    if (typeof data.lat !== "undefined") {
        if (data.lat === null || typeof data.lat === "number") lat = data.lat;
        else return errors.INVALID_LAT;
    }
    if (typeof data.lon !== "undefined") {
        if (data.lon === null || typeof data.lon === "number") lon = data.lon;
        else return errors.INVALID_LON;
    }
    // can't reset both
    if (lat !== null && lon === null) return errors.INVALID_LON;
    if (lat === null && lon !== null) return errors.INVALID_LAT;
    // store
    if (lat === null && lon === null) this.loc = null;
    else this.loc = [lat, lon];
