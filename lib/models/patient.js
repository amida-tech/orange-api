...
function createResource (collectionName) {
    return function (data, done) {
        var resource = this[collectionName].create(data);
...
PatientSchema.methods.createDoctor = createResource('doctors');

// view child resources
function findResourceById (collectionName) {
    var collectionId = collectionName + "._id";
    return function (id, done) {
        var doctor = this[collectionName].id(id);
        // no doctor found
        if (!doctor) return done(errors.INVALID_DOCTOR_ID);
        done(null, doctor);
    };
};
PatientSchema.methods.findDoctorById = findResourceById('doctors');

// remove child resources
function findResourceByIdAndDelete (collectionName) {
    return function (id, done) {
        findResourceById(collectionName).bind(this)(id, function (err, doctor) {
            if (err) return done(err);
            doctor.remove();
            this.save(function (err, patient) {
                if (err) return done(err);
...
PatientSchema.methods.findDoctorByIdAndDelete = findResourceByIdAndDelete("doctors");

// update child resources
function findResourceByIdAndUpdate (collectionName) {
    return function (id, data, done) {
        findResourceById(collectionName).bind(this)(id, function (err, doctor) {
            if (err) return done(err);

            var err = doctor.setData(data);
            if (err) return done(err);
...
PatientSchema.methods.findDoctorByIdAndUpdate = findResourceByIdAndUpdate("doctors");
