...
PatientSchema.methods.getId = function (callback) {
    mongoose.model("Counter").nextInSeq("patientId", function (err, counter) {
        if (err) return callback(err);
        this._id = counter;
        callback(null, this);
    }.bind(this));
};
PatientSchema.pre('validate', function (next, done) {
