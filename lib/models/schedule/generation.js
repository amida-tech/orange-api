...
module.exports = function (Schedule) {
    ...
    Schedule.prototype.generate = function (startExact, endExact, habits, numberTaken, userId, moreSchedules) {
        ...
        if (this.until.type === "number") {
            ...
            // console.log("GENERATE CALLED WITH start %s", startExact.format());
            // console.log("SCHEDULE START DATE %s", moment(this.date));
            ...
            if (scheduleStart === thisStart) {
                ...
                var numberLeft = Math.max(this.until.stop - numberTaken, 0); // saturate at 0
                ...
                if (events.length > 0) {
                    ...
                    var offset = s.findIndex(function (event) {
                        return (event.date === events[0].date && event.type === events[0].type && event.index === events[0].index);
                    });
                    // if not present, all doses have already been taken
                    if (typeof offset === "undefined" || offset === null || offset < 0) events = [];
                    // otherwise count that many events as already having taken place
                    else {
                        if (numberTaken !== 0 && !numbers.isNatural(numberTaken)) numberTaken = 0;
                        var numberLeft = Math.max(this.until.stop - numberTaken - offset, 0); // saturate at 0
                        events = events.slice(0, numberLeft);
                    }
                }
            }
