...
module.exports = function (Schedule) {
    ...
    Schedule.prototype.isValid = function () {
        ...
        if (this.regularly) {
            ...
            var start = this.frequency.start;
            if (typeof start !== "undefined" && start !== null) {
                if (typeof start === "string") {
                    // one start date
                    date = moment.utc(start, "YYYY-MM-DD");
                    if (!date.isValid()) return false;
                } else if (typeof start === "object" && start !== null && start.constructor === Array) {
                    // array of start dates
                    if (start.length === 0) return false; // don't allow empty arrays
                    for (var i = 0; i < start.length; i++) {
                        if (typeof start[i] !== "string") return false;
                        date = moment.utc(start[i], "YYYY-MM-DD");
                        if (!date.isValid()) return false;
                    }
                } else {
                    // invalid format
            ...
            anyInvalid = this.times.some(function(item) {
                ...
                keys = Object.keys(item).filter(function (k) {
                    return ["id", "_id", "notifications"].indexOf(k) < 0;
                });
                ...
                if (item.type === "event") {
                    ...
                    if (["before", "after", "during"].indexOf(item.when) < 0) return true;
