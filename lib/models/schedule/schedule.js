...
// This is NOT a mongoose model, but a constructor and some prototype methods for the `Schedule` type. This is because a `Schedule` is not a top level document in Mongo, but a subdocument in Medications.

// parse and validate a schedule
// * `data` is the JSON data object (received from client or mongo) that holds all the schedule information (keys are snake_case)
// * `habits` are the patient's habits. We don't use the habits themselves at all but we get the patient's timezone from this.
//
// Mostly initializes `this` like so:
//     this.camelCase = data.snake_case;
// Exception is `data.frequency.start` turns into `this.cycleStarts`. Exact `times` are converted to UTC. These are put back into whatever local time the patient is in when the schedule is generated.
// Also includes default values if certain keys don't exist in `data`. These are
//         asNeeded: data is empty ? true : false
//         regularly: false
//         date: now()
//
...
// The fields on `this` are camelCase, but our JSON representation of a `Schedule` uses snake_case for keys.
// This method returns an object with the fields in snake_case, and also removes all `notifications` from the `times` objects.
