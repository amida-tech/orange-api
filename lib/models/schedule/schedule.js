...
Schedule.prototype.toSummary = function (start, end, tz) {
    ...
    if (this.regularly) {
        ...
        var hasExcludes = typeof freq.exclude !== "undefined" && freq.exclude !== null && freq.exclude.exclude.length > 0;
        if (hasExcludes) {
            // except every 5, 6
        ...
        this.cycleStarts.forEach(function(cycleStart) {
            var cycleStart = moment.tz(cycleStart, "YYYY-MM-DD", tz).startOf("day");
            ...
            additionalInfo += cycleStart.format("MM/DD/YY");
        }.bind(this));
        additionalInfo += ")";
        ...
        if (freq.unit === "day" && freq.n === 1) {
            if (hasExcludes && freq.exclude.repeat === 7 && this.cycleStarts.length === 1) {
                ...
                var days = indices.map(function (index) {
        ...
        if (this.until.type === "number") summary += util.format(" for %d doses", this.until.stop);
            ...
            summary += util.format(" until %s", until.format("MM/DD/YY"));
        }
