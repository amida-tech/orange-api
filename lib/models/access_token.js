...
// generate and save access token for a specific user
AccessTokenSchema.statics.generateSaveAccessToken = function(user, callback) {
    this.generateAccessToken(function(err, token) {
        if (err) return callback(err);

        // save generated token
        this.create({ token: token, email: user.email }, callback);
    }.bind(this));
}
