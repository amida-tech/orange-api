...
var JournalEntrySchema = module.exports = new mongoose.Schema({
    ...
    sideEffect:         { type: String, required: false, default: "" },
    sideEffectSeverity: { type: Number, default: 1, required: false,
                          min: [1, "INVALID_SIDEEFFECT"],
                          max: [10, "INVALID_SIDEEFFECT"] },
    mood:               { type: String, required: false, default: "" },
    moodSeverity:       { type: Number, default: 1, required: false,
                          min: [1, "INVALID_MOOD"],
                          max: [10, "INVALID_MOOD"] },
...
JournalEntrySchema.methods.setData = function (data) {
    ...
    if (typeof data.user !== "undefined") this.creator = `${data.user.firstName} ${data.user.lastName}`;
