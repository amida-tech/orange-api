...
JournalEntrySchema.methods.setData = function (data) {
    ...
    if (typeof data.moodEmoji !== "undefined") {
        if(!/^\\U[\da-f]{1,8}$/i.test(data.moodEmoji))
	...
	if(typeof data.meditationLength !== "undefined"){
		...
		if(data.meditationLength === null)
			return errors.MEDITATIONLENGTH_NULL;
...
JournalEntrySchema.statics.findMeditations = function(params, cb) {
  var copy = Object.assign({}, params, { meditation : true});
  return this.find(copy, cb);
};

JournalEntrySchema.statics.findMoods = function(params, cb){
  var copy = Object.assign({}, params, { mood: true});
  return this.find(copy, cb);
};
