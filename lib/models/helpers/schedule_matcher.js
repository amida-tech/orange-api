...
ScheduleMatcher.prototype.match = function (schedule, doses, habits, callback) {
    ...
    if (this.doseTimes.length > 0) {
        firstWake = (moment.tz(this.doseTimes[0] * 1000, tz).hour(wake[0]).minute(wake[1]).second(0).millisecond(0)).format();
    ...
    function dayIndex(dose) {
        ...
        var startDay = moment.tz(dose, tz).hour(wake[0]).minute(wake[1]).second(0).millisecond(0);
        if (startDay.isAfter(moment.tz(dose, tz))) {
