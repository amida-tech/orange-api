...
ScheduleMatcher.prototype.match = function (medication, doses, habits, callback) {
    ...
    if (!_.has(habits, "wake") || habits.wake === null) {
        habits.wake = "00:00 am"; // changed to match default in models/patient/schedule.js:97
    }
    ...
    function parseDose(dose) {
        ...
        var formatted = moment(dose.date.utc).format();
        var newTz = moment.tz(formatted, tz).format();
        newDose.date.utc = newTz;
        return newDose;
    }
    function sortByDate(dose) {
        return moment.tz(dose.date.utc, tz).unix();
    }
    ...
    function dayIndex(dose) {
        ...
        var startDay = moment.tz(dose.date.utc, tz).hour(wake[0]).minute(wake[1]).second(0).millisecond(0);
        if (startDay.isAfter(moment.tz(dose.date.utc, tz))) {
