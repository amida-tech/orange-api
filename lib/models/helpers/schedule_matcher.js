...
ScheduleMatcher.prototype.match = function (schedule, doses, habits, callback) {
    ...
    function parseDose(dose) {
        dose["date"] = moment.tz(dose["date"], tz).format();
        return dose;
    }
    function sortByDate(dose) {
        return moment(dose.date).unix();
    }
    ...
    if (this.doseTimes.length > 0) {
        firstWake = (moment(this.doseTimes[0] * 1000).hour(wake[0]).minute(wake[1]).second(0).millisecond(0)).format();
        if (moment(firstWake).isAfter(moment(this.doseTimes[0] * 1000))) {
            firstWake = (moment(firstWake).subtract(1, "days")).format();
        }
    } else {
        // sensible default (TODO: I don"t think this is sensible *JSS; what about timezone?)
        firstWake = moment().format();
    }
    ...
    var ret = {
        "matches": matches,
        "start": moment(firstWake).toISOString()
    };
    callback(null, ret);
};
