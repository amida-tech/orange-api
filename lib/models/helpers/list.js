...
var defaultFilters = function (Model) {
    return {
        ...
        firstName: function (item, value) {
            var keys = Model.metaphone(value);
            // use stored metaphone values for fuzzy matching
            return intersection(item._s_name, keys).length > 0;
        },

        lastName: function (item, value) {
            var keys = Model.metaphone(value);
            // use stored metaphone values for fuzzy matching
            return intersection(item._s_name, keys).length > 0;
        },

        documentName: function (item, value) {
            var keys = Model.metaphone(value);
            // use stored metaphone values for fuzzy matching
            return intersection(item._s_documentName, keys).length > 0;
        },
        ...
        start_date: function (item, value) {
            ...
            return !moment(item.date.utc).isBefore(value);
        },

        end_date: function (item, value) {
            // date must be on or before end date
            return !moment(item.date.utc).isAfter(value);
        },
        ...
        // filter to find specific role notes
        role: function (item, role) {
            return item.role === role;
        },

        // filter to find entries with/without freeform text
        textAllowed: function (item, textAllowed) {
            return textAllowed ? true : !item.text;
        }
    };
};
...
var defaultSorters = {
    ...
    firstName: function (itemA, itemB) {
        return itemA.firstName.localeCompare(itemB.firstName);
    },

    lastName: function (itemA, itemB) {
        return itemA.lastName.localeCompare(itemB.lastName);
    },

    documentName: function (itemA, itemB) {
        return itemA.documentName.localeCompare(itemB.documentName);
    },
    ...
    date: function (itemA, itemB) {
        var dateA = moment.utc(itemA.date.utc);
        var dateB = moment.utc(itemB.date.utc);
