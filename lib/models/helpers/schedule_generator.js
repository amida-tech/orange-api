...
var numbers     = require("./numbers.js"),
    ...
    DATE_REGEXP = require("./time.js").DATE_ONLY_REGEXP;
...
var createDatetime = function (date, time) {
    date = moment(date); // don't modify original
...
ScheduleGenerator.prototype.validateParseDates = function (startRaw, endRaw) {
    ...
    var start = moment(startRaw, "YYYY-MM-DD");
    var end = moment(endRaw, "YYYY-MM-DD");
    ...
    if (typeof this.schedule.stopDate !== "undefined") {
        var stop = moment(this.schedule.stopDate, "YYYY-MM-DD"); // previously validated
...
ScheduleGenerator.prototype.generateTimesOfDay = function () {
    ...
    return function (day) {
        ...
        events.push.apply(events, times.map(function (time) {
            return createDatetime(day, time);
        }));
...
var patientSleepsLate = function (wake, sleep) {
    return (typeof wake !== "undefined" && typeof sleep !== "undefined" &&
            ...
            moment(sleep, "HH:MM").isBefore(moment(wake, "HH:MM")));
}
...
ScheduleGenerator.prototype.generateInterval = function () {
    ...
    return function (day) {
        console.log("ON DAY %s", day.format());
        // turn cycleStart and cycleEnd into real datetimes
        var start = createDatetime(day, cycleStart);
        var end = createDatetime(day, cycleEnd);
