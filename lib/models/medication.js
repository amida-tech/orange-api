...
MedicationSchema.methods.getData = function (patient) {
    ...
    if (typeof data.schedule.times !== "undefined" && data.schedule.times !== null) {
        ...
        // convert HH:MM times from UTC (stored internally) to local time (API facing)
        data.schedule.times = data.schedule.times.map(function (item) {
            if (item.type === "exact") {
                item.time = moment.utc(item.time, "HH:mm").tz(patient.tz).format("HH:mm");
            }
            return item;
        });
    }
...
MedicationSchema.methods.generateSchedule = function (start, end, patient, userId) {
    return this.schedule.generate(start, end, patient.habits, this.numDoses(patient), userId);
};
...
// give IDs to each time in the schedule, if they don't have one already
MedicationSchema.pre("save", function (next) {
    if (typeof this.rawSchedule !== "object" || this.rawSchedule === null) return next();
    var times = this.rawSchedule.times;
    if (typeof times !== "object" || times === null || times.constructor !== Array) return next();
    ...
    // and this way we preserve IDs across small schedule changes so dose matchup still works okay
...
var formatNotificationSettings = function (time, user) {
    // time.notifications.default should always exist and be numeric as we create it
    // above
    ...
    if (typeof defaultOffset !== "number") defaultOffset = 30;

    // user offset should default to "default"
    var userOffset = notifications[user._id];
    if (typeof userOffset !== "number") userOffset = "default";
...
MedicationSchema.methods.toSummary = function () {
    if (typeof this.brand === "string" && this.brand.length > 0 && typeof this.name === "string" && this.name.length > 0)
...
MedicationSchema.methods.updateNotificationSettings = function (rawTimeId, user, data, callback) {
    ...
    // rawSchedule is schemalesss so mongoose doesn't autodetect changes to it
    // (schedule is a virtual providing nicer access to rawSchedule, but rawSchedule is the only
    // thing stored in mongo)
    this.markModified("rawSchedule");
