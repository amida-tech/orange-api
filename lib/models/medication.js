...
MedicationSchema.methods.findNotificationSettings = function (rawTimeId, user, callback) {
    ...
    console.log("RETURN THE NOTIFICATION SETTINGS");
    callback(null, {});
};
...
MedicationSchema.methods.updateNotificationSettings = function (rawTimeId, user, data, callback) {
    ...
    var updatedATime = false; // check we updated something
    schedule.times = times.map(function (time) {
        if (time.id === timeId) {
            updatedATime = true;
            console.log("UPDATE THE NOTIFICAITON SETTINGS");
        }
        return time;
    });
    if (!updatedATime) return callback(errors.INVALID_TIME_ID);
