...
var MedicationSchema = module.exports = new mongoose.Schema({
    ...
    doctorId:       { type: Number, ref: "Patient.Doctor", default: null, index: true },
    pharmacyId:     { type: Number, ref: "Patient.Pharmacy", default: null, index: true },
...
MedicationSchema.methods.getData = function (patient) {
    ...
    if (typeof data.schedule.times !== "undefined" && data.schedule.times !== null) {
        ...
        // add heading and description to each time
        for (var i = 0; i < data.schedule.times.length; i++) {
            var time = data.schedule.times[i];

            // heading
            time.heading = util.format("Event %d", i + 1); // 0-indexed
            ...
            if (time.type === "unspecified") {
                ...
                var t = moment.tz(time.time, "hh:mm a", patient.tz);
                var timeFormatted;
    ...
    // if present, output patient ID
    // (used for non-patient-specific combined endpoints)
    if (typeof this.patientId !== "undefined" && this.patientId !== null) data.patient_id = this.patientId;
...
MedicationSchema.methods.generateSchedule = function (globalStart, globalEnd, patient, userId) {
    ...
    globalEnd = moment.tz(globalEnd, "YYYY-MM-DD", tz).endOf("day");
    ...
    for (var i = 0; i < this.schedules.length; i++) {
        ...
        // if this is the first regular schedule on this day, then start at the beginning
        // of the day
        // console.log("thinking about starting at start of day");
        if (lastRegularEnd === null ||
                moment.tz(lastRegularEnd, tz).format("YYYY-MM-DD") !== moment.tz(start, tz).format("YYYY-MM-DD")) {
        ...
        if (schedule.regularly) lastRegularEnd = end;
    }
    ...
    events = events.sort(function (itemA, itemB) {
        var timeA = moment(itemA.date.utc);
        var timeB = moment(itemB.date.utc);
...
MedicationSchema.methods.numDoses = function (patient) {
    ...
    return patient.doses.filter(function (dose) {
        ...
        return moment(dose.date.utc).isAfter(moment.tz(this.fillDate, patient.tz));
    }.bind(this)).length;
};
