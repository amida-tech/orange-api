...
MedicationSchema.methods.authorize = function (access, user, patient, callback) {
    ...
    if (share.group === "owner") return callback();
    ...
    if (medicationAccess === "write") return callback();

    // explicit read permissions work only if access is read
    if (medicationAccess === "read") {
        if (access === "read") return callback();
        return callback(errors.UNAUTHORIZED);
    }

    // always deny 'none'
    if (medicationAccess === "none") return callback(errors.UNAUTHORIZED);
