...
MedicationSchema.pre("save", function (next) {
    ...
    async.map(times, function (time, callback) {
        ...
        if (typeof time.id === "number") return callback(null, time);

        // use Counter model to get next ID (see helpers/increment_plugin.js for primary usage)
        mongoose.model("Counter").nextInSeq("timeId", function (err, counter) {
        ...
        this.rawSchedule.times = results;
        next();
    }.bind(this));
});
...
MedicationSchema.methods.findNotificationSettings = function (rawTimeId, user, callback) {
    ...
    if (String(timeId) !== rawTimeId || timeId < 0)
        return callback(errors.INVALID_TIME_ID);
...
MedicationSchema.methods.updateNotificationSettings = function (rawTimeId, user, data, callback) {
    ...
    schedule.times = times.map(function (t) {
        if (t.id === timeId) {
            ...
            // create a new one
            if (typeof time.notifications !== "object" || time.notifications === null)
                time.notifications = {};

            // update default
            if (typeof data.default !== "undefined") time.notifications.default = data.default;

            // update user, hashing by ID
            if (typeof data.user !== "undefined") time.notifications[user._id] = data.user;
        }
    ...
    this.parent().markModified("medications");
