...
MedicationSchema.methods.generateSchedule = function (globalStart, globalEnd, patient, userId) {
    ...
    for (var i = 0; i < this.schedules.length; i++) {
        ...
        if (start === null || start < globalStart || (start > globalStart && i === 0)) {
        ...
        var schedule = new Schedule(this.schedules[i], this.habits || {});
        // console.log("calling generate from %s to %s", start.toString(), end.toString());
        var s = schedule.generate(start, end, patient.habits, this.numDoses(patient), userId, moreSchedules);
        // console.log();
        // console.log();
        // console.log();
        // console.log();
    ...
    if (schedules.length === 0) {
        var schedule = new Schedule(this.schedules[0], this.habits || {});
        var s = schedule.generate(globalStart, globalEnd, patient.habits, this.numDoses(patient), userId, moreSchedules);
        schedules.push(s);
    }
...
MedicationSchema.virtual("schedule").get(function () {
    ...
    if (typeof oldSchedule.times === "object" && oldSchedule.times !== null && oldSchedule.times.constructor === Array) {
    ...
    if (typeof newSchedule.times === "object" && newSchedule.times !== null && newSchedule.times.constructor === Array) {
