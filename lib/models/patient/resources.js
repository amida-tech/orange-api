...
module.exports = function (PatientSchema) {
    ...
    var Pharmacy          = mongoose.model("Pharmacy", PharmacySchema),
        ...
        Event             = mongoose.model("Event", EventEntrySchema),
        DocumentSignature = mongoose.model("DocumentSignature", DocumentSignatureSchema),
    ...
    PatientSchema.methods.createEvent = create("events", Event);
    PatientSchema.methods.createEmergencyContact = create("emergencyContacts", EmergencyContact);
    PatientSchema.methods.createDocumentSignature = create("documentSignatures", DocumentSignature);
    ...
    PatientSchema.methods.findEmergencyContactById = find("emergencyContacts", errors.INVALID_EMERGENCY_CONTACT_ID);
    ...
    PatientSchema.methods.queryJournalEntries = query("entries", JournalEntry, {
        ...
        includeSkippedMeditations: function (item, includeSkippedMeditations) {
            return includeSkippedMeditations ? true : (item.meditationTaken === true || item.meditationTaken === undefined);
        }
    }, {});
    PatientSchema.methods.queryEvents = query("events", Event, {
    }, {});

    PatientSchema.methods.queryEmergencyContacts = query("emergencyContacts", EmergencyContact, {}, {});
    PatientSchema.methods.queryDocumentSignatures = query("documentSignatures", DocumentSignature, {}, {});
    ...
    function update (collectionName, invalidResourceId) {
        return function (id, data, done) {
            find(collectionName, invalidResourceId).bind(this)(id, function (err, resource) {
                ...
                // note that this subdoc save only runs save middleware, so we still need to run this.save
                // ^^^ (this is the message that is printed when suppressWarnings is false)
                resource.validate(err => {
                    ...
                    this.validate(err2 => {
                        ...
                        resource.save({ suppressWarning: true }, err3 => {
                            if (err3) return done(err3);
                            if (collectionName === "medications" && typeof data.schedule !== "undefined") {
                              this.getCreator().then((creator) => {
                                creator.sendPushNotification({
    ...
    PatientSchema.methods.findEventByIdAndUpdate = update("events", errors.INVALID_EVENT_ID);
    PatientSchema.methods.findEmergencyContactByIdAndUpdate = update("emergencyContacts", errors.INVALID_EMERGENCY_CONTACT_ID);
        ...
        return function (id, done) {
            ...
            find(collectionName, invalidResourceId).bind(this)(id, function (err, resource) {
                ...
                if (typeof removeHooks[collectionName] === "function")
                    removeHooks[collectionName](this, resource);
    ...
    PatientSchema.methods.findEventByIdAndDelete = remove("events", errors.INVALID_EVENT_ID);
    PatientSchema.methods.findEmergencyContactByIdAndDelete = remove("emergencyContacts", errors.INVALID_EMERGENCY_CONTACT_ID);
    ...
    removeHooks.doctors = function (patient, doctor) {
        patient.medications = patient.medications.map(function (medication) {
            if (medication.doctorId === doctor._id) medication.doctorId = null;
            return medication;
        });
        patient.markModified("medications");
    };
    // and likewise for pharmacies and pharmacyId on medications
    removeHooks.pharmacies = function (patient, pharmacy) {
        patient.medications = patient.medications.map(function (medication) {
            if (medication.pharmacyId === pharmacy._id) medication.pharmacyId = null;
            return medication;
        });
        patient.markModified("medications");
    };
    ...
    removeHooks.medications = function (patient, medication) {
        patient.doses = patient.doses.filter(function (dose) {
            return dose.medicationId !== medication._id;
        });
        patient.markModified("doses");
        patient.entries = patient.entries.filter(function (entry) {
            return entry.medicationIds.indexOf(medication._id) < 0;
        });
    ...
    removeHooks.doses = function (patient, dose) {
        var entryId = dose.entryId;
        if (typeof entryId !== "number") return;

        var entry = patient.entries.id(entryId);
        if (typeof entry !== "undefined" && entry !== null) entry.remove();
        patient.markModified("entries");
    };

    // when deleting a journal entry, set the notes to be blank on any associated dose
    removeHooks.entries = function (patient, entry) {
        var doseId = entry.doseId;
        if (typeof doseId !== "number") return;
    ...
    PatientSchema.methods.calcMeditationStats = function () {
        var meditations = this.entries.filter(function (entry) {
            return entry.meditation;
        });
        var totalTime = meditations.filter(function (entry) {
            return typeof entry.meditationLength !== undefined && entry.meditationLength !== null;
        }).reduce(function (sum, entry) {
            return sum + entry.meditationLength;
        }, 0);
        return {
    ...
    PatientSchema.methods.getCreator = function () {
        var User = mongoose.model("User");
        return new Promise((resolve, reject) => {
          User.findOne({ email: this.creator }, (err, data) => {
              if (err) {
                  reject(err);
              } if (data) {
                  resolve(data);
