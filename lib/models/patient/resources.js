...
var DoctorSchema        = require("../doctor.js"),
    ...
    AdherenceSchema     = require("../adherence.js");
...
module.exports = function (PatientSchema) {
    ...
    var Pharmacy        = mongoose.model("Pharmacy", PharmacySchema),
        ...
        JournalEntry    = mongoose.model("Journal", JournalEntrySchema),
    ...
    function create (collectionName, Model) {
        return function (data, done) {
            ...
            var preError = resource.setData(data);
    ...
    PatientSchema.methods.createAdherence = create("adherences", Adherence);
    ...
    PatientSchema.methods.findAdherenceById = find("adherences", errors.INVALID_ADHERENCE_ID);
    ...
    function update (collectionName, invalidResourceId) {
        return function (id, data, done) {
            find(collectionName, invalidResourceId).bind(this)(id, function (err, resource) {
                ...
                async.series([resource.validate, resource.save], function (err) {
    ...
    PatientSchema.methods.findAdherenceByIdAndUpdate = update("adherences", errors.INVALID_ADHERENCE_ID);
    ...
    PatientSchema.methods.findAdherenceByIdAndDelete = remove("adherences", errors.INVALID_ADHERENCE_ID);

    // Check all adherence medicationIds correspond to medications owned by the patient
    // We need access to the patient, so this validation has to be done here as opposed to in
    // adherence.js
    PatientSchema.path("adherences").validate(function (adherences) {
        // loop over all adherences and check they're all valid medications
        // belonging to this patient
        return adherences.every(function (adherence) {
            var medication = this.medications.id(adherence.medicationId);
