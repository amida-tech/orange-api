...
module.exports = function (PatientSchema) {
    ...
    PatientSchema.methods.createShare = function (email, access, group, callback) {
        this.share(email, access, group, function (err, patient) {
            if (err) return callback(err);
    ...
    // find share by ID
    PatientSchema.methods.findShare = function (shareId, callback) {
        // .id doesn't work as we're using numeric IDs
        var share = this.shares.filter(function (share) {
            return share._id.toString() === shareId.toString();
        })[0];
    ...
    PatientSchema.methods.removeShare = function (shareId, callback) {
        // TODO: pull not working here because we're using custom IDs?
        // instead use filter but slow!
        // first find share (so we can return it)
        var share = this.shares.filter(function (s) {
            return s._id.toString() === shareId.toString();
        })[0];
        // share must exist
        if (typeof share === "undefined" || share === null) return callback(errors.INVALID_SHARE_ID);

        // then remove it by filtering again
    ...
    PatientSchema.methods.updateShare = function (shareId, modifications, callback) {
        ...
        if (typeof modifications.access !== "undefined") share.access = modifications.access;
        if (typeof modifications.group !== "undefined") share.group = modifications.group;
