...
module.exports = function (PatientSchema) {
    ...
    // an asynchronous version of shareForEmail that also includes an is_user
    // field (representing if a user with the given email address currently exists)
    PatientSchema.methods.fullShareForEmail = function (email, callback) {
        var share = this.shareForEmail(email);
        if (typeof share === "undefined" || share === null) return callback(null, share);
    ...
    PatientSchema.methods.authorize = function (user, access, callback) {
        // TODO update to new permissions structure
