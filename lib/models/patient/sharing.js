...
module.exports = function (PatientSchema) {
    ...
    PatientSchema.methods.share = function (email, access, group, first_name, last_name, callback, avatar) {
        ...
        if (access === "none") {
            ...
            if (typeof first_name !== "undefined") share.first_name = first_name;
            if (typeof last_name !== "undefined") share.last_name = last_name;
            if (typeof avatar !== "undefined") share.avatar = avatar;
        } else {
    ...
    PatientSchema.methods.createShare = function (email, access, group, first_name, last_name, callback, avatar) {
        this.share(email, access, group, first_name, last_name, function (err) {
            if (err) return callback(err);
            callback(null, this.shareForEmail(email));
        }.bind(this), avatar);
    };

    PatientSchema.methods.getAvatarURI = function(email, callback){
       ...
       this.constructor.findOne({
           ...
           if (err) return callback(err);
           if (!patient) return callback(errors.INVALID_EMAIL); // no user found for that email
            const avatar = `/v1/patients/${patient._id}/avatar.png`;
            callback(null, avatar);
       });
   };
    // remove a share with the given shareId
    PatientSchema.methods.removeShare = function (shareId, callback) {
        // find share
