...
module.exports = function (PatientSchema, zrpc) {
    ...
    PatientSchema.methods.generateSchedule = function (start, end, user, medicationId, callback) {
        ...
        async.map(medications, function (medication, done) {
            ...
            var events = medication.generateSchedule(start, end, this);
            ...
            medication.match(doses, zrpc(), this.habits, function (err, result) {
                ...
                events = events.map(function (event) {
                    ...
                    var startDay = moment.tz(event.date, tz).hours(wake.hours()).minutes(wake.minutes()).seconds(0).milliseconds(0);
                ...
                events = events.map(function (event) {
                    ...
                    if (event.happened) {
                        ...
                        var match = result.matches.filter(function (match) {
                            return match.match !== null && match.match.day === event.day && match.match.index === event.index;
                        })[0];

                        // if a dose was taken at this schedule event
