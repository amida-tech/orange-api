...
var DATE_FORMAT = require("../helpers/time.js").DATE_FORMAT;
...
module.exports = function (PatientSchema) {
    ...
    PatientSchema.methods.generateScheduleResults = function (start, end, user, medicationId, userId, callback) {
        ...
        if (typeof start !== "string" || start.length === 0)
            start = moment.tz(today, DATE_FORMAT, tz).startOf("day"); // default to today
        else
            start = moment.tz(start, DATE_FORMAT, tz);
        if (typeof end !== "string" || end.length === 0)
            end = moment.tz(today, DATE_FORMAT, tz).add(6, "days").endOf("day"); // default to one week later
        else
            end = moment.tz(end, DATE_FORMAT, tz);
        ...
        async.map(medications, function (medication, done) {
            ...
            medication.match(doses, this.habits, function (err, result) {
                ...
                /* The default was set to 9 AM which caused medications scheduled before that to have trouble
                    associating dose events with the scheduled medication. -Jonah Bregstone 7/17/2018
                */
                var wake = moment.tz(this.habits.wake || "00:00 am", "HH:mm a", tz);
                events = events.map(function (event) {
                    ...
                    var startDay = moment.tz(event.date, tz)
                                         ...
                                         .seconds(0)
                                         .milliseconds(0);
                    if (startDay.isAfter(moment.tz(event.date, tz))) {
                ...
                events = events.map(function (event) {
                    ...
                    if (typeof match !== "undefined" && match !== null) {
                        ...
                        if (dose.taken) {
                            ...
                            event.delay = moment(dose.date.utc).seconds(0).diff(moment(event.date), "minutes");
