...
module.exports = function (PatientSchema) {
    ...
    PatientSchema.statics.findForUser = function (parameters, user, done) {
        ...
        if (typeof parameters.name !== "undefined" && parameters.name !== null && parameters.name.length !== 0) {
            // require to be any word in _s_name
            var keys = this.metaphone(parameters.name);
            if (keys.length > 0) query._s_name = { "$in": this.metaphone(parameters.name) };
        }
    ...
    PatientSchema.virtual("nameMetaphone").get(function () {
        return this.constructor.metaphone(this.name);
    });
    PatientSchema.pre("save", function (next) {
        this._s_name = this.nameMetaphone;
        next();
    });
};
