...
module.exports = function(UserSchema) {
    ...
    UserSchema.methods.makeRequest = function(email, callback) {
        ...
        var previousRequestRejected = this.requested.filter(function(request) {
            return request.email === email && request.status === "rejected";
        }).length;
        ...
        if (existing.length > 0 && !previousRequestRejected) return callback(errors.ALREADY_REQUESTED);
        ...
        this.constructor.findOne({
            ...
            if (previousRequestRejected) {
              user.requests = user.requests.map((_request) => {
                if (_request.email === this.email && _request.status === "rejected") {
                  return { email: this.email, status: "pending" };
                } else {
                  return _request;
                }
              });
            } else {
              user.requests.push({ email: this.email, status: "pending" });
            }
            ...
            request.getId(function(err) { // add numeric ID to request
                ...
                if (previousRequestRejected) {
                  this.requested = this.requested.map((_request) => {
                    if (_request.email === request.email && _request.status === "rejected") {
                      return request;
                    } else {
                      return _request;
                    }
    ...
    UserSchema.methods.cancelRequest = function(requestId, callback) {
        ...
        this.constructor.findOne({
            ...
            async.parallel({
            ...
            }, function(err) { // eslint-disable-line no-extra-bind
                if (err) return callback(err);
                ...
                // We don't want this right now, and it's broken due to sendmail package upgrade
                // user.notify({
