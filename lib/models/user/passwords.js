...
module.exports = function (UserSchema) {
    ...
    // Reset password for a certain user
    UserSchema.statics.resetPassword = function (email, callback) {
        ...
        this.findOne({ email: email }, function (err, user) {
            ...
            if (!user) return callback(errors.USER_NOT_FOUND_FOR_RESET);

            // generate a new temporary password with lowercase letters (97-122 in ascii)
            ...
            // send user an email with the new password
            user.notify({
