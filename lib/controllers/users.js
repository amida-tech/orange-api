...
if (!config.allowPublicRegistration) {
    maybeAuthenticate = [authenticate];
}
// Register a new user
users.post("/", ...maybeAuthenticate, function (req, res, next) {
    if (!config.allowPublicRegistration) {
        // only admin and programAdministrator scopes can create users
        if (!(req.authClaims.scopes.includes("admin") || req.authClaims.scopes.includes("programAdministrator"))) {
    ...
    if (process.env.NODE_ENV !== "test") {
        new User(potentialUser).validate(function(err) {
            ...
            makeRequest({
                ...
                const userData = JSON.parse(httpResponse.body);
                potentialUser.uuid = userData.uuid;

                // Add user to the messaging microservice user table
                makeRequest({
                    url: `${config.messagingServiceAPI}/user`,
                    headers: {
...
users.get("/all", authenticate, paramParser, function (req, res, next) {
