...
//Add UUID for users that dont already have one
User.find({ "UUID" : { "$exists" : false } }, function(err, users) {
  ...
  users.forEach(function(t) {
    ...
    // User.update({ _id: t._id }, { $set: { UUID: uuid4 } }, function(err) {
    //   console.log("do this");
    //   if (err) {
    //       console.log(err);
    //   }
    ...
    if (process.env.NODE_ENV !== "test") {
        new User(potentialUser).validate(function(err) {
            ...
            makeRequest({
                ...
                // const authUser = JSON.parse(httpResponse.body);
                potentialUser.uuid = JSON.parse(httpResponse.body).uuid;
                console.log(potentialUser);
                //const authUser = JSON.parse(httpResponse.body);
                //console.log("Data from user creation ", httpResponse.body, " ");f
