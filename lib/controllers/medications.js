...
var medications = module.exports = express.Router({
    mergeParams: true
});
...
medications.get("/:medicationid/times/:timeid", guard(["admin", "clinician", "user", "helpDesk"]), authorize("read"), function(req, res, next) {
    // find medication
    req.patient.findMedicationById(req.params.medicationid, function(err, medication) {
        if (err) return next(err);
...
medications.put("/:medicationid/times/:timeid", guard(["admin", "user"]), authorize("write"), filterTimesInput, function(req, res, next) {
    // find medication
    req.patient.findMedicationById(req.params.medicationid, function(err, medication) {
        if (err) return next(err);
...
medications.post("/", guard(["admin", "clinician"]), auth.authorize("write"), filterInput, function(req, res, next) {
    // Merging
    ...
    if (!req.data.brand || req.data.brand === "") {
        req.data.brand = "Generic";
    }
...
medications.get("/:medicationid", guard(["admin", "clinician", "user", "helpDesk"]), authorize("read"), function(req, res, next) {
...
medications.delete("/:medicationid", guard(["admin", "clinician"]), authorize("write"), function(req, res, next) {
...
medications.put("/:medicationid", guard(["admin", "clinician"]), authorize("write"), filterInput, function(req, res, next) {
...
medications.get("/", guard(["admin", "clinician", "user", "helpDesk"]), auth.authorize("read"), paramParser, function(req, res, next) {
