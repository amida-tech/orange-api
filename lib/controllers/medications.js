...
medications.put("/:medicationid/times/:timeid", guard(["user"]), authorize("write"), filterTimesInput, function(req, res, next) {
    // find medication
    req.patient.findMedicationById(req.params.medicationid, function(err, medication) {
        if (err) return next(err);
...
medications.post("/", guard(["admin", "clinician", "user"]), auth.authorize("write"), filterInput, function(req, res, next) {
    // Merging
