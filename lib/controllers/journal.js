...
var inputKeys = ["date", "text", "medication_ids", "medications", "mood", "moodEmoji", "meditation",
                "meditationLength"];
var keys = module.exports.keys = inputKeys.concat(["hashtags", "clinician"]);
    ...
    return function (req, res, next) {
        ...
        auth.authorize(access)(req, res, function (err) {
            ...
            req.patient.findJournalEntryById(req.params.journalentry, function (err, entry) {
                ...
                if (entry.clinician && !req.user.clinician) return next(errors.UNAUTHORIZED);
...
function setClinician(req, res, next) {
    req.data.clinician = req.user.clinician;
    return next();
}

var addMedications = function (req, res, next) {
...
var queryJournalEntries = function (meditation, mood){
 return function (req, res, next) {
    // patients cannot view clinician notes
    const clinicianParam = req.user.clinician ? {} : { clinician: false };
    const meditationParam = meditation ? {meditation: true} : {};
    const moodParam = mood ? {mood: true } : {};
    const filters = Object.assign({}, req.listParameters.filters, clinicianParam, meditationParam, moodParam);
...
journal.post("/",
    auth.authorize("write"), filterInput, authorizeFromData("write"), setClinician,
function (req, res, next) {
    req.patient.createJournalEntry(req.data, returnData(res, next));
}, formatObjectCode(201)); // return 201 status code

journal.get("/moods", auth.authorize("read"), paramParser, list.parseDateFilters, addMedications, queryJournalEntries(false, true), formatList);

//View a listing of meditation events for the specified patient
journal.get("/meditations", auth.authorize("read"), paramParser, list.parseDateFilters, addMedications, queryJournalEntries(true, false), formatList);
...
journal.put("/:journalentry",
    authorize("write"), filterInput, authorizeFromData("write"), setClinician,
function (req, res, next) {
...
journal.get("/", auth.authorize("read"), paramParser, list.parseDateFilters, addMedications, queryJournalEntries(false, false), formatList);
