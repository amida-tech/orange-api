    ...
    return function (req, res, next) {
        ...
        auth.authorize(access)(req, res, function (err) {
            ...
            req.patient.findJournalEntryById(req.params.journalentry, function (err, entry) {
                ...
                if (entry.clinician && req.user.role !== "clinician") return next(errors.UNAUTHORIZED);
...
function setClinician(req, res, next) {
    req.data.clinician = req.user.role === "clinician";
    return next();
}
...
journal.get("/", auth.authorize("read"), paramParser, list.parseDateFilters, function (req, res, next) {
    ...
    const clinicianParam = req.user.role === "clinician" ? {} : { clinician: false };
    const filters = Object.assign({}, req.listParameters.filters, clinicianParam);
