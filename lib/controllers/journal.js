...
var inputKeys = ["date", "text", "medication_ids", "medications", "mood", "moodSeverity", "moodEmoji", "meditation",
                "meditationLength", "meditationDifficulty", "meditationRequestedAssistance", "creator", "sideEffect", "sideEffectSeverity", "activity", "activityMinutes"];
...
journal.post("/helpDeskNote", guard(["helpDesk"]), auth.authorize("read"), filterInput, authorizeFromData("read"), setRole, function (req, res, next) {
    req.patient.createJournalEntry(Object.assign({}, req.data, { user: req.user }), returnData(res, next));
}, formatObjectCode(201)); // return 201 status code
