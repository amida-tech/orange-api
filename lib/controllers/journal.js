...
var inputKeys = ["date", "text", "medication_ids", "medications", "mood", "moodSeverity", "moodEmoji", "meditation",
                "meditationLength", "meditationDifficulty", "creator", "sideEffect", "sideEffectSeverity", "activity", "activityMinutes"];
...
journal.post("/", redirectIfHelpDesk("journal/helpDeskNote"),
...
journal.post("/helpDeskNote", auth.authorize("read"), filterInput, authorizeFromData("read"), setRole, function (req, res, next) {
    req.patient.createJournalEntry(Object.assign({}, req.data, { user: req.user }), returnData(res, next));
}, formatObjectCode(201)); // return 201 status code
...
journal.get("/", auth.authorize("read"), paramParser, list.parseDateFilters, function (req, res, next) {
    ...
    // patients cannot view clinician notes /* commented out */
    const filters = Object.assign({}, req.listParameters.filters, { /* role: req.user.role */ });
