...
doses.get("/:doseid", guard(["admin", "programAdministrator", "clinician", "user"]), authorize("read"), function(req, res, next) {
...
doses.get("/", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("read"), paramParser, list.parseDateFilters, function(req, res, next) {
...
doses.get("/nonempty/first", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("read"), paramParser, list.parseDateFilters, function(req, res, next) {
    // validate and parse filter query parameters
    var rawId = req.listParameters.filters.medication_id;
    if (rawId !== null) {
        // convert from string to int
