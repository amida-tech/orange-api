...
pharmacies.post("/", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("write"), filterInput, function (req, res, next) {
...
pharmacies.get("/:pharmacyid", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("read"), function (req, res, next) {
    req.patient.findPharmacyById(req.params.pharmacyid, returnData(res, next));
}, formatObject);

// Remove a single pharmacy for the specified patient
pharmacies.delete("/:pharmacyid", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("write"), function (req, res, next) {
...
pharmacies.put("/:pharmacyid", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("write"), filterInput, function (req, res, next) {
...
pharmacies.get("/", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("read"), paramParser, function (req, res, next) {
