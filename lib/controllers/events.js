...
events.post("/", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("write"), filterInput, function (req, res, next) {
    req.patient.createEvent(req.data, returnData(res, next));
}, formatObjectCode(201));


events.get("/:eventid", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("read"), function (req, res, next) {
    req.patient.findEventById(req.params.eventid, returnData(res, next));
}, formatObject);

events.delete("/:eventid", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("write"), function (req, res, next) {
    req.patient.findEventByIdAndDelete(req.params.eventid, returnData(res, next));
}, formatObject);

events.put("/:eventid", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("write"), filterInput, function (req, res, next) {
...
events.get("/", guard(["admin", "programAdministrator", "clinician", "user"]), auth.authorize("read"), paramParser, list.parseDateFilters, function (req, res, next) {
