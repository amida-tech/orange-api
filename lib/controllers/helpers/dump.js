...
module.exports = function (patient, user, startRaw, endRaw, next) {
    ...
    var addMedications = function (data, callback) {
        async.map(patient.medications.filter(function (medication) {
            ...
            output.schedule_summary = medication.schedule.toSummary(tz, start, end);

            // call schedule matcher to find adherence stats
            patient.generateSchedule(start.format("YYYY-MM-DD"), end.format("YYYY-MM-DD"), user, medication._id, user._id,
                ...
                if (medication.schedule.regularly) {
                    ...
                    for (var i = 0; i < times.length; i++) {
                        ...
                        if (time.type === "exact") {
                            time.time = moment.utc(time.time, "HH:mm").tz(tz).format("HH:mm");
                        }
        ...
        patient.generateSchedule(start.format("YYYY-MM-DD"), end.format("YYYY-MM-DD"), user, null, user._id,
            ...
            for (var date = moment(start); !end.isBefore(date); date.add(1, "day")) {
                ...
                var dayMatches = matches.schedule.map(function (item) {
                    ...
                    return (date.date() === m.date.date() && date.month() === m.date.month() && date.year() === m.date.year());
                });

                // calculate and display overall adherence stats
                day.stats = calculateAdherences(dayMatches);
                ...
                day.events = dayMatches.map(function (item) {
                    ...
                    result.time = item.date.format("HH:mm");

                    // find medication
                    var medication = patient.medications.id(item.medication_id);
                    result.medication_label = medication.toSummary(" ");
                    // don't 0-index
                    if (typeof item.scheduled !== "undefined") result.medication_label += util.format(" (Event %d)", item.scheduled + 1);
    ...
    async.seq(addPatient, addUser, addTime, addEntries, addDoctors, addPharmacies, addMedications, addSchedule, addShares, addRequested, addRequests)({}, next);
};
/*

var basicDump = function (patient, user, next) {
    // add lists of patient's doctors, pharmacies
    data.doctors = patient.doctors;
    data.pharmacies = patient.pharmacies;
