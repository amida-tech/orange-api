...
app.post('/v1/auth/token', function(req, res) {
    ...
    User.authenticate(email, password, function(err, user) {
        if (err) {
            // three cases here: invalid password, user not found and account locked
            // we output the first two as invalid_email_password and the last as
            // login_attempts_exceeded
            var error = 'invalid_email_password';
            if (err.message === 'login_attempts_exceeded') {
