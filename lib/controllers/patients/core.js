...
var initialKeys = ["first_name", "last_name", "birthdate", "sex", "access_anyone", "access_prime", "access_family",
                    "phone", "sharesCount"];
var inputKeys = initialKeys.concat(["access", "group", "meditations"]);
var keys = module.exports.keys = inputKeys.concat(["avatar", "creator", "me", "instructorEmail"]);
...
var initialFilterInput = crud.filterInputGenerator(initialKeys),
    ...
    filterInputInstructor = crud.filterInputGenerator(["instructorEmail"]),
...
patients.post("/", auth.authenticate, guard(["admin"]), initialFilterInput, function (req, res, next) {
...
// change a patients instructor
patients.put("/:id/instructor", auth.authenticate, filterInputInstructor, function (req, res, next) {
    // Requires user to have write access to patient only accepts instructorEmail as a param
    Patient.findByIdAndUpdateForUser(req.params.id, req.data, req.user, returnData(res, next));
}, formatObject);
...
patients.get("/", auth.authenticate, paramParser, function (req, res, next) {
    ...
    // admins, program admins, and help desk get access to all patient data
    if (req.user.role === "admin" || req.user.role === "programAdministrator" || req.user.role === "helpDesk")
        // the model handles the querying for us
        return Patient.findForClinician(params, req.user, returnListData(res, next));
    else
        return Patient.findForUser(params, req.user, returnListData(res, next));
