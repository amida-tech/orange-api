...
shared.post("/", auth.authenticate, guard(["admin", "programAdministator", "clinician", "user"]), auth.authorize("write"), filterInput, function (req, res, next) {
...
shared.delete("/:shareid", auth.authenticate, guard(["admin", "programAdministator", "clinician", "user"]), auth.authorize("write"), function (req, res, next) {
    // remove share and format it for output
    req.patient.removeShare(req.user, req.params.shareid, returnShare(res, next));
}, formatObject);

// update a share
shared.put("/:shareid", auth.authenticate, guard(["admin", "programAdministator", "clinician", "user"]), auth.authorize("write"), modifyFilterInput, function (req, res, next) {
...
shared.get("/", auth.authenticate, guard(["admin", "programAdministator", "clinician", "user"]), auth.authorize("read"), paramParser, function (req, res, next) {
