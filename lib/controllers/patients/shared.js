...
shared.post("/", auth.authenticate, auth.authorize("write"), filterInput, function (req, res, next) {
    ...
    //If no names provided set to unspecified
    console.log("Looking at no first_name", first_name);
    // if (first_name = )
...
var paramParser = list.parseListParameters(["id", "email"], ["is_user", "group", "access", "email"]);
