...
class ScheduleMatcher(object):
    ...
    def __init__(self, schedule, doses, habits, params):
        ...
        if len(self.doseTimes) > 0:
            ...
            # sensible default
            self.firstWake = datetime.datetime.now()

        # find all the "unspecified" schedule events we have
        self.unspecified = []
        for event in schedule["times"]:
            if event["type"] == "unspecified":
                self.unspecified.append(event["_id"])
    ...
    def match(self, debug=False):
        matches = [] # match objects to be returned
        # transpose of matrix corresponding to the above, used to determine what to do with unmatched dose
        # events
        reverse_matches = {}
        for dose in self.doses:
            ...
            # extract day of dose
            day = self.dayIndex(dose["date"])
            match["match"]["day"] = day
            if (not day in reverse_matches): reverse_matches[day] = {}

            # store ID of dose
            match["dose"] = dose["_id"]

            # use scheduled attribute if present (contains ID of index we should match to)
            if ("scheduled" in dose and dose["scheduled"] != None):
                # just in case event matched to is unspecified, record it so we don't duplicate
                reverse_matches[day][dose["scheduled"]] = True
                match["match"]["index"] = dose["scheduled"]

            # otherwise we'll loop through and match up in a second
        ...
        for i in range(len(matches)):
            ...
            if ("index" in match["match"]): continue

            # otherwise check if there's still a free "unspecified" schedule event that day to match to
            if debug:
                print colored("FINDING UNSPECIFIED MATCH FOR", "blue")
                print match
            for event in self.unspecified:
                event_used = (event in reverse_matches[day]) and (reverse_matches[day][event] == True)

                if debug: print colored("Checking schedule event: ", "yellow") + str(event) + str(event_used)
                if (not event_used):
                    reverse_matches[day][event] = True
                    match["match"]["index"] = event
                    break

            # if we still haven't found anything, use None to denote no match was found (i.e., it was probably
            # an "As Needed" medication taking dose event)
            if (not "index" in match["match"]): match["match"] = None
        ...
        return {
            "matches": matches,
            "start": self.firstWake.isoformat()
        }
